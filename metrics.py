...
def metrics_eval_single(pianoroll):
  is_eval = True
  batch_size = 64
  iter_to_save = 100
  sample_size = 64
  ...
  eval_map = np.array([
                    [1],  # metric_is_empty_bar
                    [1],  # metric_num_pitch_used
                    [1],  # metric_too_short_note_ratio
                    [1],  # metric_polyphonic_ratio
                    [1],  # metric_in_scale
                    [0],  # metric_drum_pattern
                    [1]   # metric_num_chroma_used
                ])
    
  exp_name = 'exp'
  gpu_num = '0'
  metric = Metrics(eval_map,
                  inter_pair,
                  drum_filter,
                  scale_mask,
                  track_names,
                  SEQ_LEN,
                  NUM_NOTES)
  metric.eval(pianoroll[:,:,:,:])

def metrics_eval(pianoroll):
  is_eval = True
  batch_size = 64
  iter_to_save = 100
  sample_size = 64
